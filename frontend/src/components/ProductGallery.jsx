...
export const ProductGallery = ({ product, testId = "product-gallery", compact = false, items, coverId }) => {
  ...
  useEffect(() => { if (!full) return; const onKey = e => { if (e.key === "Escape") setFull(false); }; document.addEventListener("keydown", onKey); return () => document.removeEventListener("keydown", onKey); }, [full]);
  ...
  return <div data-testid={testId} className="min-w-0">
    ...
    {full && <div data-testid={`${testId}-lightbox`} role="dialog" aria-modal="true" aria-label="Foto produk ukuran penuh" className="fixed inset-0 z-[260] flex items-center justify-center bg-black/90 p-5" onClick={() => setFull(false)}>
      <button type="button" data-testid={`${testId}-zoom-close`} aria-label="Tutup foto" className="absolute right-5 top-5 rounded bg-white p-2" onClick={() => setFull(false)}><X /></button>
