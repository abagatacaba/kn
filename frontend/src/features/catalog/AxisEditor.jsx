...
export const AxisEditor = ({ axes, onChange, structuralLocked = false }) => {
  const [label, setLabel] = useState("");
  const [key, setKey] = useState("");
  const update = (idx, patch) => onChange(axes.map((a, i) => i === idx ? { ...a, ...patch } : a));
  const add = (key0, label0) => {
    if (axes.some(a => a.key === key0) || !key0 || !label0) return;
    onChange([...axes, { key: key0, label: label0, options: [] }]); setKey(""); setLabel("");
  };
  return <section data-testid="catalog-axis-editor" className="space-y-4 border-t pt-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold" data-testid="axis-section-title">Atribut & pilihan varian</h3>
      {!structuralLocked && <div className="flex flex-wrap gap-1">{[["color", "Warna"], ["grade", "Grade"], ["lebar", "Lebar"], ["size", "Ukuran"], ["material", "Material"]].map(([k, l]) => <button type="button" key={k} data-testid={`axis-add-${k}`} className="secondary-button" disabled={axes.some(a => a.key === k) || axes.length >= 6} onClick={() => add(k, l)}><Plus size={12} />{l}</button>)}</div>}
    </div>
    ...
    {axes.map((a, idx) => <div key={a.key} data-testid={`axis-editor-${a.key}`} className="border-l-2 border-[#DCE7F7] pl-3">
      <div className="mb-2 flex items-center justify-between"><span data-testid={`axis-name-${a.key}`} className="text-sm font-semibold">{a.label} <span className="text-xs font-normal text-gray-500">{a.key === "lebar" ? "(cm)" : ""}</span></span>
...
const OptionInput = ({ axis, onAdd }) => {
  ...
  const add = () => { if (!text.trim() || !code || axis.options.some(o => o.code === code || o.label === text.trim())) return; onAdd({ label: axis.key === "lebar" ? `${text.trim()}cm` : text.trim(), code, value: axis.key === "lebar" ? Number(text.replace(",", ".")) / 100 : "", hex: "" }); setText(""); setCode(""); };
